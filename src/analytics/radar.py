...
def _add_derived_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "return_on_capital_pct" not in result.columns and all(
        c in result.columns
        for c in ("operating_profit", "depreciation", "equity_capital", "reserves", "borrowings")
    ):
...
def add_composite_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column, weight in WINSORISED_METRICS.items():
        ...
        if sector is not None:
            scored = series.groupby(sector).transform(_winsorised_score)
        else:
            scored = _winsorised_score(series)
        score += weight * scored
        total += weight
...
def scored_universe(db_path: str | Path = DB_PATH) -> pd.DataFrame:
    """Latest-year ratios with derived metrics and composite score."""
    return add_composite_score(latest_year_frame(_add_derived_metrics(load_financial_ratios(db_path))))
...
def main() -> None:
    ...
    active = results.get(config.get("active_preset") or "", next(iter(results.values()), pd.DataFrame()))
    if not active.empty:
        cols = [c for c in ("company_id", "sector", "composite_quality_score") if c in active.columns]
